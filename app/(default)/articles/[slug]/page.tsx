...
async function fetchData(slug: string) {
  const response = await http.get<GetPublishedArticleResponse>(`public/articles/${slug}`, { next: { revalidate: 60 } });
    ...
    <>
      <div className="min-h-screen bg-gray-900 text-white">
        <div className="p-6">
          ...
          {/* Article Abstract */}
          <p className="mt-4 text-sm text-gray-400">{article.abstract}</p>

          {/* Article Content */}
          <div className="mt-6 text-white" dangerouslySetInnerHTML={{ __html: article.content }} />
          ...
          <div className="mt-6">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {article.tags.map((tag) => (
                <span key={tag} className="rounded-full bg-gray-700 px-3 py-1 text-xs text-white">
                  {tag}
                </span>
              ))}
            </div>
